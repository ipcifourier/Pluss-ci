<x-layout>
    ...
    <div class="relative bg-gray-900 h-64 md:h-80 flex items-center overflow-hidden">
        ...
        {{-- Image de fond (Cover) --}}
        @if ($gtt->cover_image)
            <img src="{{ asset('images/' . $gtt->cover_image) }}"
        ...
        <div
            class="container mx-auto px-4 relative z-10 text-center md:text-left flex flex-col md:flex-row items-center gap-8 mt-8 md:mt-0">

            {{-- Logo GTT flottant --}}
            <div class="bg-white p-2 rounded-full shadow-lg shrink-0">
                @if ($gtt->logo)
                    <img src="{{ asset('images/' . $gtt->logo) }}" class="w-24 h-24 rounded-full object-contain">
                @else
    ...
    <div class="container mx-auto px-4 py-12 grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div class="lg:col-span-4 space-y-8">
                        ...
                        @foreach ($relatedDocuments as $doc)
                            <li class="py-3 first:pt-0 last:pb-0">
                                <a href="{{ route('documents.download', $doc) }}"
                                    class="group block hover:bg-gray-50 p-2 -mx-2 rounded transition">
                                    <span
                                        class="text-sm font-semibold text-gray-700 group-hover:text-green-600 line-clamp-2">{{ $doc->title }}</span>
                                    <span class="text-xs text-gray-400 mt-1 block">Publié le
                            ...
                            <div class="flex gap-4">
                                ...
                                @if ($article->image_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($article->image_path))
                                    <img src="{{ asset('images/' . $article->image_path) }}"
                                        class="w-16 h-16 rounded-lg object-cover shrink-0 group-hover:scale-105 transition">
                                @else
                                    <div
                                        class="w-16 h-16 rounded-lg bg-orange-100 flex items-center justify-center shrink-0">
                                ...
                                <div>
                                    <div class="flex items-center gap-2 mb-1">
                                        <span
                                            class="text-xs font-bold text-orange-500 uppercase tracking-wider">Info</span>
                                        <span
                                            class="text-xs text-gray-400">{{ $article->published_at ? $article->published_at->format('d/m/Y') : $article->created_at->format('d/m/Y') }}</span>
                                    </div>
                                    <h4
                                        class="text-sm font-bold text-gray-800 leading-snug group-hover:text-orange-500 transition-colors">
            ...
            <div class="bg-orange-500 rounded-xl p-8 text-white text-center shadow-lg relative overflow-hidden group">
                <div
                    class="absolute inset-0 bg-orange-600 w-0 group-hover:w-full transition-all duration-500 ease-out z-0">
                </div>
                <div class="relative z-10">
                    <h3 class="font-bold text-xl mb-2">Besoin d'information ?</h3>
                    <p class="text-orange-100 text-sm mb-6">Contactez le secrétariat technique de ce groupe de travail.
                    </p>
                    <a href="{{ route('contact') }}"
                        class="inline-block bg-white text-orange-600 font-bold py-3 px-8 rounded-full hover:shadow-xl hover:scale-105 transition transform shadow-sm">
                        Nous contacter
                    </a>
                </div>
            </div>
